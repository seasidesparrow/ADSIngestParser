...
class BITSAffils(object):
    ...
    def parse(self, article_metadata):
        ...
        # BITS puts author data in <contrib-group>, giving individual authors in each <contrib>
...
class BITSParser(BaseBeautifulSoupParser):
    def __init__(self):
    ...
    def _parse_title_abstract(self):
        ...
        book_title_group = self.article_meta.find("book-title-group")
        if not title_group and book_title_group:
            title_group = book_title_group
            self.doctype = "book"
        art_title = None
        sub_title = None
        if title_group:
            title_type = "article-title"
            if self.doctype == "book":
                title_type = "book-title"
            
            if title_group.find(title_type):
                title = title_group.find(title_type)
                if not title or not title.get_text():
                    title = title_group.find("alt-title")
                if not title or not title.get_text():
                    title = title_group.find("title")
    ...
    def parse(self, text, bsparser="lxml-xml"):
        """
        Parse BITS XML into standard JSON format
        ...
        self.collection_meta = d.find("collection-meta")
        if not self.collection_meta:
            self.collection_meta = d.find("book-meta")
        self.article_meta = self.collection_meta
        self.journal_meta = self.collection_meta
        self.back_meta = d.back
